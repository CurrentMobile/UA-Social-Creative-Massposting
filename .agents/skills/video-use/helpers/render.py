...
# -------- Alpha WebM overlays (HyperFrames transparent overlays) -------------
#
# HyperFrames renders transparent overlays as VP9 WebM with an alpha plane.
# ffmpeg's DEFAULT native vp9 decoder silently drops that alpha plane, so the
# overlay decodes as opaque (yuv420p) and its transparent regions composite as
# solid colour — covering the base video. ffmpeg's `libvpx-vp9` decoder DOES
# read the alpha plane (yuva420p), and the `overlay` filter then blends it
# correctly. So we detect alpha WebM inputs and force `-c:v libvpx-vp9` on that
# specific `-i` in build_final_composite — no pre-transcode to ProRes needed.
#
# Detection is two-pronged because the native decoder hides the alpha:
#   - the WebM container advertises ALPHA_MODE=1 (present regardless of decoder), or
#   - the probed stream pix_fmt already names an alpha format (e.g. yuva420p).


def _pix_fmt_has_alpha(pix_fmt: str) -> bool:
    """True if an ffmpeg pixel-format name denotes an alpha channel."""
    pf = pix_fmt.lower()
    return (
        pf.startswith("ya")          # ya8, ya16le
        or "yuva" in pf              # yuva420p, yuva444p10le, …
        or "rgba" in pf or "bgra" in pf
        or "argb" in pf or "abgr" in pf
        or "gbra" in pf              # gbrap, gbrapf32le
        or "ayuv" in pf
    )


def webm_has_alpha(path: Path) -> bool:
    """Return True if a .webm overlay carries an alpha (transparency) plane.

    Non-.webm inputs always return False (cheap short-circuit). See the module
    comment above for why this matters and how the alpha is recovered.
    """
    ...
    for line in out.splitlines():
        ...
        val = val.strip()
        if key == "tag:alpha_mode" and val == "1":
            return True
        if key == "pix_fmt" and _pix_fmt_has_alpha(val):
            return True
    return False
...
def build_final_composite(
    ...
    for ov in overlays:
        ...
        if webm_has_alpha(ov_path):
            # Force libvpx-vp9 for THIS input so the WebM alpha plane is decoded
            # (yuva420p). With the default vp9 decoder ffmpeg drops alpha and the
            # overlay's transparent regions render opaque, covering the base.
    ...
    if has_subs:
        # Forward-slash the path first so ffmpeg's subtitles filter doesn't strip
        # Windows backslashes (libass treats "\U", "\A"… as escapes). Harmless on POSIX.
    ...
    alpha_note = f" ({alpha_overlays} alpha WebM via libvpx-vp9)" if alpha_overlays else ""
    print(f"  overlays: {len(overlays)}{alpha_note}, subtitles: {'yes' if has_subs else 'no'}")
    subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
